...
public class CharacterCarousel : MonoBehaviour
{
    ...
    [Header("Highlight")]
    public float selectedScaleBoost = 1.15f;
    public float dimAlpha = 0.5f;
    public float normalAlpha = 1f;

    [Header("Premium Highlight")]
    public float interactableThreshold = 0.2f;
    void Start()
    {
    ...
    void ApplyVisuals()
    {
        for (int i = 0; i < characters.Length; i++)
        {
            ...
            // 🎨 DIM + INTERACTABLE CONTROL
            CanvasGroup cg = characters[i].GetComponent<CanvasGroup>();
            if (cg != null)
            {
                float targetAlpha = Mathf.Lerp(1f, 0.5f, distance);
                cg.alpha = Mathf.Lerp(cg.alpha, targetAlpha, Time.deltaTime * moveSpeed);

                // ✅ Only center is clickable
                bool isFront = distance < 0.2f;

                cg.interactable = isFront;
                cg.blocksRaycasts = isFront;
            }
        }
    }
}